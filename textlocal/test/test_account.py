...
class TemplateTest(unittest.TestCase):
    ...
    def test_template(self):
        title = 'title'
        body = 'body'
        template = textlocal.account.Template(title, body)
...
class GroupTest(unittest.TestCase):
    ...
    def test_group_with_only_name(self):
        name = 'name'
        group = textlocal.account.Group(name)
    ...
    def test_group_with_group_id(self):
        name = 'name'
        group_id = 1
        group = textlocal.account.Group(name, group_id)
        self.assertEqual(group.id, group_id)

    def test_group_with_size(self):
        name = 'name'
        size = 1
        group = textlocal.account.Group(name, size=size)
